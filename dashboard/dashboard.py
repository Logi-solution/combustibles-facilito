...
COLOR_PRODUCTO = {
    "Gasolina Regular": "#1D9E75",
    "Gasolina Premium": "#378ADD",
    "Diesel DB5": "#D85A30",
}
...
st.markdown(
    ...
    .rank-row {display:flex; justify-content:space-between; font-size:12px; padding:2px 0; border-bottom:1px solid rgba(128,128,128,0.15);}
...
def render_sidebar_header():
    st.sidebar.markdown(
        """
        <div style="height:30vh; display:flex; flex-direction:column; justify-content:center; gap:12px;">
          <div style="border:1px solid rgba(128,128,128,0.35); border-radius:8px; padding:16px; text-align:center;
                      font-size:12px; color:var(--text-secondary,#888);">
            Logo Logi Solution
          </div>
          <div>
            <div style="font-size:1.05rem; font-weight:600;">Análisis de precios de combustibles</div>
            <div style="font-size:0.8rem; color:var(--text-secondary,#888); margin-top:6px;">
              La Libertad · Ica · Piura · Lima · Lambayeque
            </div>
            <div style="font-size:0.72rem; color:var(--text-secondary,#888); margin-top:10px;">
            ...
            <div style="font-size:0.68rem; color:var(--text-secondary,#888); margin-top:6px;">
              Datos actualizados diariamente a la 1:00 pm (hora Perú).
            </div>
            <div style="font-size:0.68rem; color:var(--text-secondary,#888); margin-top:4px;">
...
k1.metric(f"Estaciones activas ({label_fecha})", f"{activas:,}")
k2.metric(f"Precio promedio · {producto}", f"S/ {dia_scope_fuel['Precio_Soles_Galon'].mean():.2f}" if not dia_scope_fuel.empty else "—")
k3.metric("Más barato", f"S/ {dia_scope_fuel['Precio_Soles_Galon'].min():.2f}" if not dia_scope_fuel.empty else "—")
k4.metric("Más caro", f"S/ {dia_scope_fuel['Precio_Soles_Galon'].max():.2f}" if not dia_scope_fuel.empty else "—")
    ...
    with st.container(border=True):
        ...
        for prod in PRODUCTOS:
            ...
            if not serie.empty:
                fig.add_trace(
                    go.Scatter(
                        x=serie["Fecha"], y=serie["Precio_Soles_Galon"], mode="lines+markers",
